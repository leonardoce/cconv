...
import it.interfree.leonardoce.iconv.jhlabs_map.AngleFormat;
...
public abstract class Projection implements Cloneable {
     ...
     */
    public final Punto2D transform(double lon, double lat, Punto2D dst) {
     ...
     */
    public Punto2D transformRadians(Punto2D src, Punto2D dst) {
        double x = src.x;
        if (projectionLongitude != 0) {
            x = MapMath.normalizeLongitude(x - projectionLongitude);
        }
     ...
     * Bernhard Jenny, May 2010.
     */
    public final Punto2D transformRadians(double lon, double lat, Punto2D dst) {
    ...
    public void transform(double[] srcPoints, int srcOffset, double[] dstPoints, int dstOffset, int numPoints) {
        Punto2D in = new Punto2D();
        Punto2D out = new Punto2D();
     ...
     */
    public Punto2D inverseTransformRadians(Punto2D src, Punto2D dst) {
        double x = (src.x - totalFalseEasting) / totalScale;
        double y = (src.y - totalFalseNorthing) / totalScale;
        projectInverse(x, y, dst);
        if (dst.x < -Math.PI) {
     ...
     */
    public void inverseTransformRadians(double srcX, double srcY, Punto2D dst) {
     ...
     */
    public Punto2D projectInverse(double x, double y, Punto2D dst) {
        dst.x = x;
        dst.y = y;
        return dst;
    }
     ...
     */
    public Rettangolo2D inverseTransform(Rettangolo2D r) {
        Punto2D in = new Punto2D();
        Punto2D out = new Punto2D();
        Rettangolo2D bounds = null;
        if (isRectilinear()) {
            for (int ix = 0; ix < 2; ix++) {
                ...
                for (int iy = 0; iy < 2; iy++) {
                    ...
                    inverseTransform(in, out);
                    if (ix == 0 && iy == 0) {
                        bounds = new Rettangolo2D(out.x, out.y, 0, 0);
                    } else {
    ...
    public void testBinarySearchInverse() {
        Punto2D pt1 = new Punto2D();
        Punto2D pt2 = new Punto2D();
     ...
     */
    protected void binarySearchInverse(double x, double y, Punto2D lp) {
    ...
    protected void binarySearchInverse(double x, double y,
            double lon, double lat, Punto2D lp) {
     ...
     */
    public Rettangolo2D transform(Rettangolo2D r) {
        Punto2D in = new Punto2D();
        Punto2D out = new Punto2D();
        Rettangolo2D bounds = null;
        if (isRectilinear()) {
